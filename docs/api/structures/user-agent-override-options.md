...
* `platform` string (optional) - Overrides `navigator.platform`
  ...
  no override.
* `hideChrome` boolean (optional) - If `true`, hide `window.chrome` in webpage
  script contexts (`typeof chrome === 'undefined'`). Does not affect
  extension or content-script worlds. Default is `false`. Can be changed later
  via another `setUserAgent` call (runtime).
